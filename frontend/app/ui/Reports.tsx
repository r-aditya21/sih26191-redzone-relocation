...
import {BarChart3,Bell,Calendar,ChevronDown,CircleHelp,Clock,Download,FileText,Home,Layers3,LogOut,MapPinned,Menu,Search,Shield,SlidersHorizontal,Users,X} from "lucide-react";
...
<nav className="space-y-1">{nav.map(([label,Icon]:any)=><button key={label} onClick={()=>{setMobileOpen(false);onNavigate?.(label)}} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-left ${label==="Reports"?"bg-forest text-white":"hover:bg-surface"}`}><Icon size={17}/>{label}{label==="Reports"&&<span className="ml-auto text-[10px] px-1.5 rounded bg-white/20">{reports.length}</span>}</button>)}</nav>
